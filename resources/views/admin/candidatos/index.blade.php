        ...
        <tr>
          ...
          <td class="nowrap">
            <div class="toolbar" style="display:flex; gap:12px">
              <a class="btn smol" href="{{ route('admin.candidatos.inscricoes', $c) }}">Inscrições</a>
              <form method="POST" action="{{ route('admin.candidatos.impersonate', $c) }}"
                    onsubmit="return confirm('Entrar como {{ addslashes($c->nome) }}?');" style="display:inline">
                @csrf
                <button type="submit" class="btn smol">Entrar como</button>
              </form>
