...
<div class="card-min">
  ...
  <div class="form-min">
    ...
    @php
      ...
      try {
        if ($dn instanceof \Illuminate\Support\Carbon) {
          $dn = $dn->format('d/m/Y');
        } elseif (is_string($dn) && trim($dn) !== '') {
          // Aceita 'Y-m-d' ou 'Y-m-d H:i:s' ou outros formatos parsáveis
          $dn = \Illuminate\Support\Carbon::parse($dn)->format('d/m/Y');
        }
      } catch (\Throwable $e) {
        // mantém como veio se não for possível converter
      }
    @endphp
