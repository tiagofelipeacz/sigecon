...
@section('title', 'Editar Cidade de Prova - SIGECON')

@php
  // Espera-se: $concurso, $cidade (->cidade, ->uf), $ufs, $cargos (id,titulo), $selecionados (array ids)
@endphp
...
<div class="gc-page">
  ...
  <div class="grid" style="gap:14px">
    <div class="gc-card">
      <div class="gc-body">
        <h5 style="margin:0 0 10px; font-weight:700">Editar Cidade de Prova</h5>
        ...
        <form method="post" action="{{ route('admin.concursos.cidades.update', [$concurso, $cidade->id]) }}">
          ...
          <div class="grid2">
            <div>
              ...
              <input type="text" name="cidade" class="input" value="{{ old('cidade', $cidade->cidade) }}" required>
            </div>
            ...
            <div>
              ...
              <select name="uf" class="select" required>
                @foreach($ufs as $k=>$v)
                  @php $selectedUf = old('uf', $cidade->uf); @endphp
                  <option value="{{ $k }}" @selected($selectedUf===$k)>{{ $v===''?'UF':$v }}</option>
                @endforeach
              </select>
            </div>
          </div>
          ...
          <div style="margin-top:18px">
            ...
            @php $checked = old('cargos', $selecionados ?? []); @endphp
