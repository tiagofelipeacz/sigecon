...
class CidadeProvaController extends Controller
{
    ...
    public function destroy(Concurso $concurso, $id)
    {
        ...
        $q = DB::table($pivot)->where('cidade_id', $id);
        if (Schema::hasColumn($pivot, 'concurso_id')) {
    ...
    protected function cidadeCargosIds(int $cidadeId, int $concursoId): array
    {
        ...
        $q = DB::table($pivot)->where('cidade_id', $cidadeId);
        if (Schema::hasColumn($pivot,'concurso_id')) {
            $q->where('concurso_id', $concursoId);
        }
        return $q->pluck('cargo_id')->map(fn($v)=>(int)$v)->all();
    }
    ...
    /** Sincroniza pivot cidade x cargos (compatível com pivot com/sem concurso_id) */
