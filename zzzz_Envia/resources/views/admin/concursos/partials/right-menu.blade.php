@php
    ...
    // Helper para gerar URL com fallback se a route não existir
    ...
    /**
     * Cálculo de "active" sem colisão com a Visão Geral:
     * - path atual, ex: admin/concursos/8/config/editar
     * - prefixo do concurso: admin/concursos/8
     * - primeira parte depois do prefixo é a "key" ativa: (vazio)=>root, "config", "vagas", etc.
     */
    ...
    // Itens do menu com chave única para "active"
    $items = [
        ['key'=>'root',          'label'=>'Visão Geral',             'icon'=>'bar-chart-3',   'url'=> $r('admin.concursos.show', [$id])],
