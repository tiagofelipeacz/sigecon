...
class InscricaoController extends Controller
{
    public function index()
    {
        ...
        $inscricoes = CandidatoInscricao::where('candidato_id', $user->id)->orderByDesc('id')->get();
    ...
    public function create()
    {
        ...
        return view('candidato.inscricoes.create', compact('concursos'));
    }

    public function cargos($concursoId)
    {
        $cargos = DB::table('concursos_vagas_cargos')->where('concurso_id', $concursoId)->orderBy('nome')->get();
    ...
    public function store(Request $request)
    {
        ...
        $cargo = DB::table('concursos_vagas_cargos')->where('id', $data['cargo_id'])->where('concurso_id', $concurso->id)->first();
        abort_unless($cargo, 404);

        $item = null;
        $localidade_id = null;
        if (!empty($data['item_id'])) {
            $item = DB::table('concursos_vagas_itens')->where('id', $data['item_id'])->where('concurso_id', $concurso->id)->where('cargo_id', $cargo->id)->first();
            abort_unless($item, 404);
            $localidade_id = $item->localidade_id;
        }

        // Impede duplicidade: um candidato por concurso
        $ja = CandidatoInscricao::where('candidato_id', $user->id)->where('concurso_id', $concurso->id)->exists();
        if ($ja) {
            return redirect()->route('candidato.inscricoes.index')->withErrors(['general'=>'Você já possui inscrição neste concurso.']);
        }

        // Protocolo: AAAAMMDD-CCC-######## (CCC = id concurso)
        $seq = (int) ($concurso->sequence_inscricao ?? 1);
        $protocolo = now()->format('Ymd') . '-' . str_pad((string)$concurso->id, 3, '0', STR_PAD_LEFT) . '-' . str_pad((string)$seq, 6, '0', STR_PAD_LEFT);
        ...
        DB::table('concursos')->where('id', $concurso->id)->update(['sequence_inscricao' => $seq + 1]);

        return redirect()->route('candidato.inscricoes.show', $insc->id)->with('success','Inscrição realizada com sucesso.');
    }

    public function show($id)
    {
        $user = Auth::guard('candidato')->user();
        $insc = CandidatoInscricao::where('id', $id)->where('candidato_id', $user->id)->firstOrFail();
        ...
        if ($insc->localidade_id) {
            $localidade = DB::table('concursos_vagas_localidades')->where('id', $insc->localidade_id)->first();
        }

        return view('candidato.inscricoes.show', compact('insc','concurso','cargo','localidade','user'));
    }
    ...
    public function comprovante($id)
    {
        ...
        if (class_exists(\Barryvdh\DomPDF\Facade\Pdf::class)) {
            $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('candidato.inscricoes.comprovante_pdf', compact('insc','concurso','cargo','localidade','user'));
            return $pdf->download('comprovante_'.$insc->protocolo.'.pdf');
        }

        // Fallback: renderiza HTML imprímivel
        return view('candidato.inscricoes.comprovante_html', compact('insc','concurso','cargo','localidade','user'));
    }
}
